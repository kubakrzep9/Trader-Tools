...
public class GUI extends Calculations{
    ...
    // Sets all button styles, text, event actions, etc. Is called in the buildGUI method
    ...
    // Updates numShares and positionPrice to their new values. Is executed when the
    // user presses the 'Round Shares' button when active.
    ...
    private void calculationFunction(Boolean boolSkipCommission){
        ...
        if(boolCond1 && boolCond2){
            ...
            else { btnRoundShares.setDisable(true); }
        }
    ...
    private void booleanGGMLLogic(){
        ...
        if(boolCond1 && boolCond2){
            if(boolML){ calculateSLinfo(); setSLinfo(); }
            if(boolGG){ calculateTPinfo(); setTPinfo(); }
